...
<script type="text/javascript">
	...
	$(function(){
		...
		$.ajax({
			type : 'POST',
			url : '${pageContext.request.contextPath}/common/siteAction!getCurrentSite.action',
			data : '',
			dataType : 'json',
			success : function(data) {
				if (data.success) {
	...
	function _change() {
		...
		$("#jsp_user_login_vCode").attr("src", "${pageContext.request.contextPath}"+"/verifyCodeAction!getVerifyCode.action?" + new Date().getTime());
	}
</script>
</head>
<body>
<div id="user_login_dialog" class="easyui-dialog"
	data-options="title:'用户登录',iconCls:'ext-icon-lock_open',resizable:true,modal:true,width:370,height:202,
		...
		<table class="table">
			...
			<tr>
              ...
              <td>
                ...
                <img id="jsp_user_login_vCode" class="verifyCode" src="<c:url value='/verifyCodeAction!getVerifyCode.action'/>" onclick="javascript:_change()"/>
                <a id="jsp_user_login_verifyCode1" href="javascript:_change()">换张图</a>
              </td>
            </tr>
		</table>
	</form>
